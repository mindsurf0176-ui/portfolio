  ...
  return (
    <div className="min-h-screen bg-background text-textMain selection:bg-primary/20 selection:text-primary relative">
      ...
      <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full px-4 flex justify-center">
        <nav className="glass-panel px-1.5 py-1.5 rounded-full flex items-center gap-1 shadow-lg shadow-black/5 max-w-[calc(100vw-2rem)] overflow-x-auto no-scrollbar touch-pan-x">
          <a
            href="#"
          ...
          <div className="hidden sm:block w-px h-4 bg-black/10 mx-1"></div>
          {navItems.map((item) => (
            <a
              key={item.id}
              href={`#${item.id}`}
              className={`relative px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-300 whitespace-nowrap ${activeSection === item.id
                  ? 'text-textMain'
